...
export const getDailyAttendance = async (req, res) => {
  try {
    ...
    // 2️⃣ Get attendance for the date
    ...
    const result = employees.map((emp) => {
      ...
      if (leaveMap[emp.userId?.toString()]) {
        status = "On Leave";
...
export const getAttendanceStats = async (req, res) => {
  try {
    ...
    /* 1️⃣ Fetch same data used by getDailyAttendance */
    ...
    const approvedLeaves = await Request.find({
      requestType: "LEAVE",
      status: "APPROVED",
      "details.fromDate": { $lte: new Date(date + "T23:59:59.999Z") },
      "details.toDate": { $gte: new Date(date + "T00:00:00.000Z") },
    });

    /* 2️⃣ Build quick lookup maps */
    const attendanceMap = {};
    attendanceRecords.forEach((r) => {
      attendanceMap[r.employee.toString()] = r;
    });

    const leaveMap = {};
    approvedLeaves.forEach((l) => {
      leaveMap[l.userId.toString()] = true;
    });
    ...
    /* 4️⃣ Apply SAME priority logic as table */
