/**
 * backfill-biometric-timestamps.mjs (Leptis branch)
 *
 * PURPOSE:
 *   Corrects the `timestamp` field on every existing BiometricTransaction record.
 *   These were originally stored via `new Date(rawData.authDateTime)` on a naive
 *   "YYYY-MM-DD HH:mm:ss.SSSSSS" string with no timezone offset - JS parses that as
 *   LOCAL time of whichever machine ran the sync, not UAE/Al Ain time (+04:00), so every
 *   stored timestamp is off by (server's local offset - 4h). This recomputes each
 *   record's `timestamp` from its own `rawData.authDateTime` using the same fixed
 *   +04:00 anchor the live sync now uses (biometricSyncService.js's
 *   parseLeptisTimestamp), so it stays correct no matter what machine this runs on.
 *
 ...
 *   as-is. Records with no `rawData.authDateTime` to recompute from are left untouched
...
import { parseLeptisTimestamp } from "../src/services/biometricSyncService.js";
...
async function run() {
  ...
  for await (const doc of cursor) {
    scanned++;
    const rawAuthDateTime = doc.rawData?.authDateTime;
  ...
  console.log(`  No rawData.authDateTime: ${noRawAuthDateTime} (left untouched)`);
